...
Apify.main(async () => {
    ...
    const { password, sessionConfig, username, userAgent, website, proxyConfiguration } = input;
    if (!((_a = input.steps) === null || _a === void 0 ? void 0 : _a.length)) {
    ...
    const crawler = new Apify.PuppeteerCrawler({
        ...
        handlePageFunction: async ({ page, request, response }) => {
            ...
            for (const step of input.steps) {
                ...
                if (step.username) {
                    await functions_1.throwIfMissing(page, step.username, "username");
                    await functions_1.focusAndType(page, step.username, username);
                }
                if (step.password) {
                    await functions_1.throwIfMissing(page, step.password, "password");
                    await functions_1.focusAndType(page, step.password, password);
                }
                // new URL throws on invalid URL, but it
                // should never happen in the real world
                currentUrl = new URL(page.url());
                const race = functions_1.waitForPageActivity(page, currentUrl, step.waitForMillis);
                if (step.submit) {
                    await functions_1.throwIfMissing(page, step.submit, "submit");
                ...
                try {
                    ...
                    log.info(`Considering page settled after ${step.waitForMillis ||
                        5000}ms`, {
                        step,
                        request
                    });
                ...
                if (step.failed) {
                    try {
                        ...
                        await functions_1.throwIfMissing(page, step.failed, "failed");
                    }
                    catch (e) {
                ...
                if (step.success) {
                    await functions_1.throwIfMissing(page, step.success, "success");
                }
